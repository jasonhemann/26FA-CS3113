...
19. The cartesian-product is defined over a non-empty list of
...
mathematics, known as the [Collatz
Conjecture](https://en.wikipedia.org/wiki/Collatz_conjecture), as to
whether, for every positive integer n, the function power limit of f
on n is 1. Your task is to, complete the below definition of collatz.
collatz should be a function which will, when given a positive integer
as an input, operate in a manner similar to the straightforward
...
#| Just Dessert (No credit no work fun problem just to try) |#
